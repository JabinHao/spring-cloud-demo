...
public class UserServiceImpl implements UserService {
    ...
    public UserServiceImpl(UserMapper userMapper, AddressMapper addressMapper, RestTemplate restTemplate) {
    ...
    public UserDTO findInfo(String uid) {
        ...
        String url = String.format("http://%s/order/get/by/uid/%s", ORDER_SERVICE_NAME, uid);
...
//        restTemplate.exchange(url, log.info("Response Received as " + response + " -  " + new Date()));
    ...
    public int saveUser(UserDTO userDTO) {
        ...
        if (orderDTOList != null && orderDTOList.size() != 0) {
            String url = String.format("http://%s/order/save", ORDER_SERVICE_NAME);
